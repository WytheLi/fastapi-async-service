...
async def user_activity_write(
    request: Request, activity_data: UserActivitySchema, session: AsyncSession = Depends(get_async_session)
):
    data = {"event_type": EventType.USER_ACTIVITY.value, "content": activity_data.model_dump()}
    await kafka_producer.send_message(settings.KAFKA_TOPIC, data)
    return StandardJSONResponse(stat_code.SUCCESS)
